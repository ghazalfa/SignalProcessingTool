...
impl FirHighpass{
    ...
    pub fn new(n_taps: usize, cutoff: f32) -> Self  {
        ...
        FirHighpass {
           filter:  FIR::highpass(n_taps,cutoff as f64)
         }

    }

    pub fn run_highpass(&mut self, input: RawData) -> Vec<f32>{
    ...
    pub fn filtfilt(&mut self, input: RawData) -> Vec<f32>{
        ...
        let mut filterclone: FirHighpass = FirHighpass::new(n_taps, cutoff);
        
        //filtering the data
        let mut filtered_data: Vec<f32> = self.run_highpass(input);
        ...
        let mut filtered_data = filterclone.run_highpass(filtered_data);
