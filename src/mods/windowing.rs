    ...
    pub fn windowing(frame: &RawData, increment: i32) -> Vec<Vec<f32>> {
        match frame {
            ...
            RawData::FloatVec(vector) => {
                let sub_vectors: Vec<Vec<f32>> = vector
...
mod tests {
    ...
    fn test_windowing() {
        ...
        assert_eq!(
            Windowing::windowing(&float_vector1, 2),
            vec![vec![6.2, 7.1], vec![6.2]]
        );
        assert_eq!(
            Windowing::windowing(&int_vector1, 2),
    ...
    fn test_rectangular() {
        ...
        assert_eq!(
            Windowing::rectangular(&float_vector1, 3),
            vec![1.0, 1.0, 1.0]
        );
    ...
    #[test]
    fn test_blackman() {
        let float_vector1: RawData = RawData::FloatVec(vec![1.0; 5]);
